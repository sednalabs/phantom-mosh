...
enum class Kind : unsigned char { hello = 1, challenge, confirm, ready, data, close, closed };
Token random_token()
{
  Token value {};
...
struct Wiped
{
  ...
  ~Wiped() { if ( !bytes.empty() ) OPENSSL_cleanse( bytes.data(), bytes.size() ); }
};
...
UdpEndpoint UdpEndpoint::parse( std::string_view host, std::uint16_t port )
{
  ...
  if ( inet_pton( AF_INET, text.c_str(), out.address_.data() ) == 1 ) {
    ...
    if ( std::all_of( a.begin(), a.end(), []( unsigned char b ) { return b == 0; } )
         || a[0] == 0xff || ( a[0] == 0xfe && ( a[1] & 0xc0 ) == 0x80 )
         || ( first_ten_zero && a[10] == 0xff && a[11] == 0xff ) )
...
struct RemoteSession::Impl
{
  ...
  Impl( Bootstrap&& root, std::uint64_t time, RemotePolicy p, Policy c )
    : crypto( Role::server, std::move( root ), time, c ), policy( p ), created( time ), now( time ), last_activity( time )
  {
    if ( !p.startup_ms || p.startup_ms > 120000 || !p.path_ms || p.path_ms > 120000
         || !p.drain_ms || p.drain_ms > 10000 )
      throw Error( "invalid remote session lifetime policy" );
  }
  ~Impl() { stop(); }
  void stop() noexcept
  {
    crypto.close(); candidate.reset(); peer.reset();
    OPENSSL_cleanse( proof.data(), proof.size() ); state = RemoteState::closed;
  }
  void tick( std::uint64_t time )
  {
    if ( time < now ) throw Error( "session clock moved backwards" );
    now = time;
    if ( state == RemoteState::closed ) return;
    crypto.tick( time );
    ...
    if ( candidate && elapsed( time, candidate->started, policy.path_ms ) ) candidate.reset();
  }
};
...
void RemoteSession::stop() noexcept { impl_->stop(); }
void RemoteSession::tick( std::uint64_t now ) { impl_->tick( now ); }
RemoteState RemoteSession::state() const noexcept { return impl_->state; }
std::optional<UdpEndpoint> RemoteSession::peer() const { return impl_->peer; }
int RemoteSession::wait_ms( std::uint64_t now ) const
{
  const auto& s = *impl_;
  if ( now < s.now ) throw Error( "session clock moved backwards" );
  if ( s.state == RemoteState::closed ) return 0;
  std::uint64_t wait = 1000;
  auto limit = [&]( std::uint64_t start, std::uint64_t duration ) {
    if ( duration ) wait = std::min( wait, now - start >= duration ? std::uint64_t{0} : duration - ( now - start ) );
  };
  if ( s.state == RemoteState::pending ) limit( s.created, s.policy.startup_ms );
  if ( s.state == RemoteState::active ) limit( s.last_activity, s.policy.idle_ms );
  if ( s.state == RemoteState::draining ) limit( s.drain_started, s.policy.drain_ms );
  if ( s.candidate ) limit( s.candidate->started, s.policy.path_ms );
...
RemoteEvent RemoteSession::receive( const UdpEndpoint& source, const Bytes& packet, std::uint64_t now )
{
  auto& s = *impl_;
  s.tick( now );
  if ( s.state == RemoteState::closed ) return {};
  try {
    ...
    auto reply = [&]( Kind kind, const Token& token ) {
      ...
      if ( wire.size() > packet.size() ) throw Error( "session amplification invariant failed" );
      return Outbound { source, std::move( wire ) };
    };
    if ( s.state == RemoteState::draining ) {
      if ( m.kind == Kind::close && active_source ) return { reply( Kind::closed, s.proof ), {}, false, false, false };
      return {};
    }
    if ( m.kind == Kind::hello && opened->newest ) {
      ...
      auto candidate = std::make_unique<Impl::Candidate>( Impl::Candidate { source, m.token, random_token(), now } );
    ...
    if ( m.kind == Kind::confirm ) {
      if ( s.candidate && s.candidate->peer == source && equal( s.candidate->challenge, m.token ) && opened->newest ) {
        ...
        s.peer = source; s.proof = m.token; s.candidate.reset();
        s.state = RemoteState::active; s.last_activity = now;
...
{
  auto& s = *impl_; s.tick( now );
  if ( s.state != RemoteState::active || !s.peer ) throw Error( "remote session is not active" );
  if ( payload.size() > SESSION_MAX_DATA ) throw Error( "session payload exceeds record budget" );
  try { return { *s.peer, seal( s.crypto, Kind::data, s.proof, now, payload ) }; }
  catch ( ... ) { s.stop(); throw; }
}
...
struct SessionClient::Impl
{
  ...
  void stop() noexcept
  {
    crypto.close(); state = ClientState::closed; have_challenge = false;
  ...
  void tick( std::uint64_t now )
  {
    ...
    try { crypto.tick( now ); } catch ( ... ) { stop(); throw; }
  }
  Bytes encrypt( Kind kind, const Token& token, std::uint64_t now, const Bytes& data = {} )
  {
    try { return seal( crypto, kind, token, now, data ); }
    catch ( ... ) { stop(); throw; }
  }
  ~Impl() { stop(); }
};
...
void SessionClient::tick( std::uint64_t now ) { impl_->tick( now ); }
void SessionClient::stop() noexcept { impl_->stop(); }
ClientState SessionClient::state() const noexcept { return impl_->state; }
const UdpEndpoint& SessionClient::server() const noexcept { return impl_->server; }
Bytes SessionClient::retry( std::uint64_t now )
{
  auto& s = *impl_;
  if ( s.state == ClientState::closed ) throw Error( "client session closed" );
  if ( s.state == ClientState::closing ) return s.encrypt( Kind::close, s.proof, now );
  if ( s.state == ClientState::active ) throw Error( "client session is already active" );
  ...
  if ( s.have_challenge && ++s.retries % 4 != 0 ) return s.encrypt( Kind::confirm, s.proof, now );
  return s.encrypt( Kind::hello, s.request, now );
}
void SessionClient::revalidate_path()
{
  auto& s = *impl_;
  if ( s.state == ClientState::closing || s.state == ClientState::closed ) throw Error( "client session closing" );
  const auto request = random_token(); s.request = request;
  s.have_challenge = false; s.retries = 0; s.state = ClientState::connecting;
...
{
  auto& s = *impl_;
  if ( source != s.server || s.state == ClientState::closed ) return {};
  s.tick( now );
  auto opened = [&]() {
    try { return s.crypto.open( packet, now ); }
    catch ( ... ) { s.stop(); throw; }
  }();
  if ( !opened ) return {};
  Wiped plain( std::move( opened->payload ) );
  ...
  if ( m.kind == Kind::challenge && s.state == ClientState::connecting && opened->newest ) {
    ...
    s.proof = m.token; s.have_challenge = true;
    return { std::move( answer ), {} };
  }
  if ( !s.have_challenge || !equal( m.token, s.proof ) ) return {};
  if ( m.kind == Kind::ready && s.state == ClientState::connecting && opened->newest ) s.state = ClientState::active;
  if ( m.kind == Kind::closed && s.state == ClientState::closing ) s.stop();
...
{
  auto& s = *impl_;
  if ( s.state != ClientState::active ) throw Error( "client session is not active" );
  if ( payload.size() > SESSION_MAX_DATA ) throw Error( "session payload exceeds record budget" );
...
{
  auto& s = *impl_;
  if ( s.state != ClientState::active && s.state != ClientState::closing ) throw Error( "client session is not active" );
  auto packet = s.encrypt( Kind::close, s.proof, now ); s.state = ClientState::closing; return packet;
}
} // namespace phantom
