...
{
  if ( !p.connect_ms || p.connect_ms > 120000 || !p.close_ms || p.close_ms > 10000
       || !p.retry_ms || p.retry_ms > 1000 || p.retry_max_ms < p.retry_ms || p.retry_max_ms > 30000
       || p.revalidate_ms > 120000 || !p.receive_budget || p.receive_budget > 64 )
...
SessionChannel::SessionChannel( SessionSocket&& socket, std::uint64_t now, ChannelPolicy policy )
  : socket_( std::make_unique<SessionSocket>( std::move( socket ) ) ), policy_( policy ),
    now_( now ), phase_started_( now ), retry_started_( now ), retry_interval_( policy.retry_ms )
{}
std::unique_ptr<SessionChannel> SessionChannel::client( SessionSocket&& socket, UdpEndpoint peer,
  Bootstrap&& secret, std::uint64_t now, ChannelPolicy policy, Policy crypto )
{
...
std::unique_ptr<SessionChannel> SessionChannel::server( SessionSocket&& socket, Bootstrap&& secret,
  std::uint64_t now, RemotePolicy lifetime, ChannelPolicy policy, Policy crypto )
{
  Bootstrap owned = std::move( secret );
  validate( policy );
  if ( socket.fd() < 0 )
...
SessionChannel::~SessionChannel() { stop(); }
void SessionChannel::finish( ChannelEnd reason ) noexcept
{
  if ( end_ != ChannelEnd::none ) return;
...
void SessionChannel::stop() noexcept { finish( ChannelEnd::stopped ); }
int SessionChannel::fd() const noexcept { return socket_ ? socket_->fd() : -1; }
ChannelState SessionChannel::state() const noexcept
{
  if ( end_ != ChannelEnd::none ) return ChannelState::closed;
  if ( client_ ) {
    switch ( client_->state() ) {
      case ClientState::connecting: return ChannelState::connecting;
      case ClientState::active: return ChannelState::active;
      case ClientState::closing: return ChannelState::closing;
      case ClientState::closed: return ChannelState::closed;
    }
  }
  if ( server_ ) {
    switch ( server_->state() ) {
      case RemoteState::pending: return ChannelState::connecting;
      case RemoteState::active: return ChannelState::active;
      case RemoteState::draining: return ChannelState::closing;
      case RemoteState::closed: return ChannelState::closed;
    }
  }
...
{
  if ( end_ != ChannelEnd::none ) return std::nullopt;
  if ( client_ ) return client_->server();
  return server_ ? server_->peer() : std::nullopt;
}
void SessionChannel::check_time( std::uint64_t now ) const
{
  if ( now < now_ ) throw Error( "channel clock moved backwards" );
}
...
{
  check_time( now );
  if ( state() == ChannelState::closed || immediate_ ) return 0;
  std::uint64_t wait = 1000; // local key-retirement tick; no idle network traffic
  if ( server_ ) wait = static_cast<std::uint64_t>( server_->wait_ms( now ) );
    ...
    const auto duration = state() == ChannelState::closing ? policy_.close_ms
      : established_ ? policy_.revalidate_ms : policy_.connect_ms;
    if ( duration ) wait = std::min( wait, remaining( now, phase_started_, duration ) );
...
ChannelBatch SessionChannel::service( std::uint64_t now )
{
  ...
  try {
    advance_time( now );
    if ( end_ != ChannelEnd::none ) return batch;
    immediate_ = false;
    ...
    while ( batch.datagrams_read < policy_.receive_budget ) {
      ...
      if ( client_ ) {
        ...
        if ( event.reply ) {
          ...
          retry_started_ = now; retry_interval_ = policy_.retry_ms; retry_sent_ = true;
        }
        if ( event.data ) batch.data.push_back( std::move( *event.data ) );
        batch.activated |= before == ChannelState::connecting && state() == ChannelState::active;
        if ( state() == ChannelState::active ) established_ = true;
        if ( client_->state() == ClientState::closed ) finish( ChannelEnd::peer_closed );
      } else {
        auto event = server_->receive( packet.peer, packet.bytes, now );
        if ( event.reply ) socket_->send( event.reply->peer, event.reply->datagram );
        if ( event.data ) batch.data.push_back( std::move( *event.data ) );
        batch.activated |= event.activated;
        batch.migrated |= event.migrated;
      }
      if ( end_ != ChannelEnd::none ) break;
    }
    ...
    if ( end_ == ChannelEnd::none && client_
      ...
      retry_started_ = now; retry_sent_ = true;
    }
    return batch;
  } catch ( ... ) { finish( ChannelEnd::failed ); throw; }
}
bool SessionChannel::send( const Bytes& payload, std::uint64_t now )
{
  check_time( now );
  if ( payload.size() > max_payload() ) throw Error( "channel payload exceeds datagram budget" );
  try { advance_time( now ); } catch ( ... ) { finish( ChannelEnd::failed ); throw; }
  if ( state() != ChannelState::active ) throw Error( "channel is not active" );
  try {
    if ( client_ ) return socket_->send( client_->server(), client_->send( payload, now ) );
...
{
  check_time( now );
  if ( !client_ ) throw Error( "only the client initiates graceful session close" );
  try { advance_time( now ); } catch ( ... ) { finish( ChannelEnd::failed ); throw; }
  if ( state() == ChannelState::closed || state() == ChannelState::closing ) return;
  if ( state() != ChannelState::active ) throw Error( "channel is not active" );
  try {
    auto output = client_->close( now );
    phase_started_ = now; retry_started_ = now; retry_interval_ = policy_.retry_ms; retry_sent_ = true;
...
void SessionChannel::rebind( SessionSocket&& socket, std::uint64_t now )
{
  ...
  now_ = now;
  if ( was_active ) phase_started_ = now;
  retry_started_ = now; retry_interval_ = policy_.retry_ms; retry_sent_ = false;
  immediate_ = true;
}
} // namespace phantom
