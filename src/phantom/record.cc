...
void require( bool good )
{
  if ( !good )
    throw Error( "cryptographic provider failure" );
}
...
void put( unsigned char* out, std::uint64_t n, std::size_t size )
{
  for ( std::size_t i = size; i != 0; --i ) {
    out[i - 1] = static_cast<unsigned char>( n );
    n >>= 8;
  }
}
...
namespace detail {
void extract( const unsigned char* key,
              std::size_t key_len,
...
void expand( const unsigned char key[32],
...
Bytes encrypt( const unsigned char key[32],
               ...
               const Bytes& plaintext )
{
  ...
  int used = 0;
  if ( length )
    require( EVP_EncryptUpdate( ctx.get(), out.data(), &used, plaintext.data(), length ) == 1 );
...
std::optional<Bytes> decrypt( const unsigned char key[32],
                              const unsigned char nonce[12],
                              const Bytes& aad,
                              const Bytes& ciphertext )
{
  ...
  int used = 0;
  if ( length )
    require( EVP_DecryptUpdate( ctx.get(), out.bytes.data(), &used, ciphertext.data(), length ) == 1 );
  Secret<16> tag;
  ...
  if ( EVP_DecryptFinal_ex( ctx.get(), out.bytes.data() + used, &n ) != 1 )
    return std::nullopt;
...
{
  OPENSSL_cleanse( text_.data(), text_.size() );
}
...
Bootstrap::Bootstrap( Bootstrap&& other ) noexcept : secret_( other.secret_ ), valid_( other.valid_ )
{
  other.clear();
}
...
{
  if ( !valid_ )
    throw Error( "bootstrap already consumed" );
  Secret<45> buffer;
...
{
  if ( text.size() != 43 )
    throw Error( "invalid bootstrap encoding" );
  constexpr std::string_view alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  for ( char c : text )
    if ( alphabet.find( c ) == std::string_view::npos )
      throw Error( "invalid bootstrap encoding" );
  if ( ( alphabet.find( text.back() ) & 3U ) != 0 )
    throw Error( "noncanonical bootstrap encoding" );
  Secret<44> padded;
  std::copy( text.begin(), text.end(), padded.bytes.begin() );
  padded.bytes[43] = '=';
  Secret<33> decoded;
...
struct Session::Impl
{
  ...
  void time( std::uint64_t t )
  {
    ...
    if ( t < now )
      throw Error( "monotonic clock moved backwards" );
    now = t;
    if ( rx_previous && t - previous_started >= policy.previous_key_ms )
      rx_previous.reset();
  }
  [[noreturn]] void exhausted()
  {
    retire();
    throw Error( "session limit reached; fresh authenticated bootstrap required" );
  }
};
Session::Session( Role role, Bootstrap&& bootstrap, std::uint64_t now_ms, Policy policy )
{
  Bootstrap owned = std::move( bootstrap );
  if ( !owned.valid() || ( role != Role::client && role != Role::server ) )
    throw Error( "invalid session initialization" );
  if ( !policy.rekey_packets || policy.hard_packets < policy.rekey_packets
       || policy.hard_packets > ( std::uint64_t { 1 } << 24 ) || !policy.rekey_ms || !policy.previous_key_ms
       || policy.previous_key_ms > 120000 || !policy.failed_verifications
       || policy.failed_verifications > ( std::uint64_t { 1 } << 32 ) ) {
  ...
  detail::extract(
    owned.secret_.data(), 32, reinterpret_cast<const unsigned char*>( salt ), sizeof( salt ) - 1, master.data() );
  owned.clear();
  ...
  state->tx_next = state->tx->next();
  state->tx->secret.clear();
  state->rx_next = state->rx->next();
  state->rx->secret.clear();
...
Bytes Session::seal( const Bytes& payload, std::uint64_t now_ms )
{
  ...
  if ( payload.size() > MAX_PAYLOAD )
    throw Error( "payload exceeds datagram budget" );
  ...
  if ( due && s.ack == s.tx->number ) {
    ...
    s.tx = std::move( s.tx_next );
    s.tx->secret.clear();
    s.tx_next = std::move( successor );
    s.tx_started = now_ms;
    s.tx_count = 0;
  }
  if ( s.tx_count >= s.policy.hard_packets || s.tx_pn > MAX_PACKET_NUMBER )
    s.exhausted();
  const auto pn = s.tx_pn++;
  ++s.tx_count; // burn even on provider/network failure
  Bytes aad( 8 );
  put( aad.data(), pn, 8 );
  WipedBytes plain( 8 + payload.size() );
  put( plain.bytes.data(), s.tx->number, 4 );
  put( plain.bytes.data() + 4, s.rx->number, 4 );
...
std::optional<Received> Session::open( const Bytes& datagram, std::uint64_t now_ms )
{
  ...
  s.time( now_ms );
  s.attempts = 0;
  if ( datagram.size() < RECORD_OVERHEAD || datagram.size() > MAX_DATAGRAM )
    return std::nullopt;
  ...
  for ( auto* candidate : candidates ) {
    ...
    if ( !decrypted ) {
      if ( ++s.failures >= s.policy.failed_verifications )
        s.exhausted();
      continue;
    }
    ...
    if ( pn > MAX_PACKET_NUMBER || epoch != candidate->number || peer_ack > s.tx->number || !s.replay.fresh( pn )
         || ( advancing && s.replay.initialized && pn <= s.replay.highest )
         || ( candidate == s.rx_previous.get() && pn >= s.epoch_first_seen ) )
      return std::nullopt;
    ...
    if ( advancing )
      successor = s.rx_next->next();
    if ( advancing ) {
      s.rx_previous = std::move( s.rx );
      s.previous_started = now_ms;
      s.rx = std::move( s.rx_next );
      s.rx->secret.clear();
      s.rx_next = std::move( successor );
