...
public:
  ...
  int release() { int old = value_; value_ = -1; return old; }
private:
  int value_;
};
...
std::string ssh_local_host()
{
  ...
  if ( !text || std::char_traits<char>::length( text ) > 256 ) throw Error( "SSH connection context required" );
  std::istringstream fields( text ); std::string client, client_port, server, server_port, extra;
  if ( !( fields >> client >> client_port >> server >> server_port ) || fields >> extra ) throw Error( "invalid SSH connection context" );
  const auto cp = number( client_port ), sp = number( server_port );
  if ( !cp || cp > 65535 || !sp || sp > 65535 ) throw Error( "invalid SSH connection context" );
...
void write_frame( int fd, std::string_view frame )
{
  ...
  while ( !frame.empty() ) {
    ...
    if ( written <= 0 ) throw Error( "session startup write failed" );
...
int daemon_main( int offer_fd, const std::string& host, RemotePolicy policy )
{
  try {
    ...
    write_frame( offer.get(), frame.view() ); frame.clear(); ::close( offer.release() );
    while ( !stopping ) {
      ...
      if ( owner->state() == ChannelState::closed || stopping ) break;
    ...
    owner->stop(); return 0;
  } catch ( ... ) { return 1; } // Never write secret-bearing diagnostics from the daemon.
}
void detach_child( int pipe_fd, const std::string& host, RemotePolicy policy )
{
  ...
  if ( pipe_fd != 3 && ::dup2( pipe_fd, 3 ) < 0 ) ::_exit( 1 );
  if ( ::syscall( SYS_close_range, 4U, ~0U, 0U ) < 0 ) ::_exit( 1 );
  int null = ::open( "/dev/null", O_RDWR | O_CLOEXEC );
  if ( null < 0 ) ::_exit( 1 );
  for ( int target = 0; target < 3; ++target ) if ( ::dup2( null, target ) < 0 ) ::_exit( 1 );
  if ( null > 3 ) ::close( null );
  struct sigaction action {}; action.sa_handler = stop_signal; ::sigemptyset( &action.sa_mask );
  for ( int sig : { SIGTERM, SIGINT, SIGHUP } ) if ( ::sigaction( sig, &action, nullptr ) != 0 ) ::_exit( 1 );
  // Returning from daemon_main destroys secret-bearing owners before _exit.
  const int status = daemon_main( 3, host, policy ); ::_exit( status );
}
void launch( const std::string& host, RemotePolicy policy )
{
  ...
  int ends[2]; if ( ::pipe2( ends, O_CLOEXEC ) != 0 ) throw Error( "session startup pipe failed" );
  Fd read_end( ends[0] ), write_end( ends[1] );
  const auto child = ::fork();
  if ( child < 0 ) throw Error( "session process creation failed" );
  if ( child == 0 ) { ::close( read_end.release() ); detach_child( write_end.release(), host, policy ); ::_exit( 1 ); }
  ::close( write_end.release() );
  int status = 0;
  while ( ::waitpid( child, &status, 0 ) < 0 ) if ( errno != EINTR ) throw Error( "session child wait failed" );
  if ( !WIFEXITED( status ) || WEXITSTATUS( status ) != 0 ) throw Error( "session startup failed" );
  SessionOfferDecoder decoder;
  struct Buffer { std::array<char, SESSION_OFFER_MAX + 1> data {}; ~Buffer(){ OPENSSL_cleanse( data.data(), data.size() ); } } buffer;
  const auto started = session_time_ms();
  for ( ;; ) {
    const auto spent = session_time_ms() - started;
    if ( spent >= 10000 ) throw Error( "session startup timed out" );
    pollfd event { read_end.get(), POLLIN, 0 };
    const int ready = ::poll( &event, 1, static_cast<int>( 10000 - spent ) );
    if ( ready < 0 && errno == EINTR ) continue;
    if ( ready <= 0 || event.revents & ( POLLERR | POLLNVAL ) ) throw Error( "session startup pipe failed" );
    const auto count = ::read( read_end.get(), buffer.data.data(), buffer.data.size() );
    if ( count < 0 && errno == EINTR ) continue;
    if ( count < 0 ) throw Error( "session startup pipe failed" );
    if ( !count ) break;
  ...
  auto frame = make_session_offer( offer.server, offer.secret ); write_frame( STDOUT_FILENO, frame.view() );
...
{
  try {
    if ( argc < 3 || std::string_view( argv[1] ) != "--session-profile" || std::string_view( argv[2] ) != SESSION_PROFILE )
    ...
    for ( int i = 3; i < argc; i += 2 ) {
      if ( i + 1 == argc ) throw Error( "missing session lifetime option" );
      const std::string_view option( argv[i] ); const auto value = number( argv[i + 1] );
      if ( option == "--startup-ms" ) policy.startup_ms = value;
      else if ( option == "--path-ms" ) policy.path_ms = value;
      else if ( option == "--drain-ms" ) policy.drain_ms = value;
      else if ( option == "--idle-ms" ) policy.idle_ms = value;
      else throw Error( "unknown session lifetime option" );
    }
    ...
    if ( !policy.startup_ms || policy.startup_ms > 120000 || !policy.path_ms || policy.path_ms > 120000
         || !policy.drain_ms || policy.drain_ms > 10000 ) throw Error( "invalid session lifetime policy" );
    launch( ssh_local_host(), policy ); return 0;
