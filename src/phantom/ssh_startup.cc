...
struct Child
{
  ...
  bool exited_successfully()
  {
    ...
    if ( ::waitid( P_PID, static_cast<id_t>( pid ), &info, WEXITED | WNOHANG | WNOWAIT ) < 0 ) {
      ...
      if ( errno == ECHILD ) owned = false; // do not signal a possibly recycled PID
...
{
  std::string out = "'";
  for ( char c : text ) { if ( c == '\'' ) out += "'\\''"; else out += c; }
  return out + "'";
}
void validate( const SshStartupOptions& o )
{
  ...
  if ( !printable( o.destination ) || o.destination.size() > 1024 || o.destination[0] == '-'
       || o.destination.find_first_not_of( allowed ) != std::string::npos
       || !printable( o.ssh_path ) || o.ssh_path[0] != '/' || !printable( o.server_path )
       || o.server_path[0] == '-' || ( !o.config_path.empty() && !printable( o.config_path ) )
       || o.timeout.count() < 1 || o.timeout.count() > 120000 || o.cancel_fd < -1 )
...
void cancelled( int fd )
{
  ...
  if ( result < 0 && errno != EINTR ) checked( false );
  if ( event.revents & POLLNVAL ) throw Error( "invalid SSH cancellation descriptor" );
  if ( event.revents ) throw Error( "SSH startup cancelled" );
}
} // namespace
StartupOffer start_over_ssh( const SshStartupOptions& options )
{
  ...
  for ( const char* setting : { "BatchMode=yes", "StrictHostKeyChecking=yes", "NumberOfPasswordPrompts=0",
                               "RequestTTY=no", "ForkAfterAuthentication=no", "SessionType=default",
                               "ControlMaster=no", "ControlPersist=no", "ClearAllForwardings=yes",
                               "ForwardAgent=no", "ForwardX11=no", "PermitLocalCommand=no",
                               "RemoteCommand=none", "UpdateHostKeys=no", "LogLevel=ERROR" } ) {
    args.emplace_back( "-o" ); args.emplace_back( setting );
  }
  if ( !options.config_path.empty() ) { args.emplace_back( "-F" ); args.push_back( options.config_path ); }
  args.emplace_back( "--" ); args.push_back( options.destination );
  args.push_back( "exec " + quote( options.server_path ) + " '--startup-profile' " + quote( std::string( PROFILE_ID ) ) );
  ...
  input.above_stdio(); output.above_stdio();
  Descriptor null( ::open( "/dev/null", O_RDWR | O_CLOEXEC ) );
  checked( null.get() >= 0 ); null.above_stdio();
  ...
  for ( int sig : { SIGPIPE, SIGINT, SIGTERM, SIGHUP, SIGQUIT } ) ::sigaddset( &defaults, sig );
  ...
  checked( ::posix_spawnattr_setflags( &attr.value, POSIX_SPAWN_SETPGROUP | POSIX_SPAWN_SETSIGMASK | POSIX_SPAWN_SETSIGDEF ) == 0 );
  Child child;
  pid_t spawned = -1;
  checked( ::posix_spawn( &spawned, options.ssh_path.c_str(), &actions.value, &attr.value, argv.data(), environ ) == 0 );
  child.pid = spawned;
  output.close();
  ...
  for ( ;; ) {
    ...
    if ( now >= deadline ) throw Error( "SSH startup timed out" );
    const bool success = child.exited_successfully();
    if ( success && eof ) return decoder.finish();
    ...
    if ( ready < 0 ) { if ( errno == EINTR ) continue; checked( false ); }
    cancelled( options.cancel_fd );
    if ( std::chrono::steady_clock::now() >= deadline ) throw Error( "SSH startup timed out" );
    if ( events[0].revents & ( POLLERR | POLLNVAL ) ) checked( false );
    if ( !( events[0].revents & ( POLLIN | POLLHUP ) ) ) continue;
    const auto count = ::read( input.get(), buffer.data.data(), buffer.data.size() );
    if ( count < 0 ) {
      if ( errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK ) continue;
      checked( false );
    }
    if ( count == 0 ) { eof = true; input.close(); continue; }
