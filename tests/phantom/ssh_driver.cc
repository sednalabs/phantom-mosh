...
int main( int argc, char** argv )
{
  ...
  options.ssh_path = argv[1]; options.destination = argv[2]; options.server_path = argv[3];
  options.config_path = argv[4]; options.timeout = std::chrono::milliseconds( std::stoi( argv[5] ) );
  options.cancel_fd = std::stoi( argv[6] );
  const bool exchange = std::string( argv[7] ) == "udp";
  if ( std::getenv( "PHANTOM_TEST_IGNORE_SIGCHLD" ) ) ::signal( SIGCHLD, SIG_IGN );
  ...
  if ( std::getenv( "PHANTOM_TEST_CLOSED_STDIN" ) ) ::close( STDIN_FILENO );
  ...
  try {
    ...
    if ( exchange ) {
      ...
      struct Socket { int fd = ::socket( AF_INET, SOCK_DGRAM | SOCK_CLOEXEC, 0 ); ~Socket() { if ( fd >= 0 ) ::close( fd ); } } socket;
      sockaddr_in address {};
      ...
      if ( ::poll( &event, 1, 2000 ) != 1 ) throw phantom::Error( "test UDP reply timed out" );
      phantom::Bytes reply( phantom::MAX_DATAGRAM + 1 );
      const auto size = ::recv( socket.fd, reply.data(), reply.size(), 0 );
      if ( size < 0 ) throw phantom::Error( "test UDP read failed" );
      reply.resize( static_cast<std::size_t>( size ) );
      const auto opened = session.open( reply, 0 );
      if ( !opened || opened->payload != payload ) throw phantom::Error( "test UDP authentication failed" );
    }
    std::cout << "accepted\n";
  ...
  if ( ::waitpid( -1, nullptr, WNOHANG ) != -1 || errno != ECHILD ) return 3;
  if ( options.cancel_fd >= 0 && ::fcntl( options.cancel_fd, F_GETFD ) < 0 ) return 4;
  if ( descriptors_before < 0 || open_descriptors() != descriptors_before ) return 5;
  return result;
}
