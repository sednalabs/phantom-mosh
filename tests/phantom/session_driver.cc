...
SessionOffer from_pipe()
{
  ...
  for ( ;; ) {
    ...
    if ( count < 0 ) throw Error( "test offer read failed" );
    if ( count == 0 ) return decoder.finish();
...
void await_state( SessionClient& client, SessionSocket& socket, ClientState wanted )
{
  ...
  while ( client.state() != wanted ) {
    ...
    if ( now >= end ) throw Error( "test client timed out" );
    if ( now >= next ) { socket.send( client.server(), client.retry( now ) ); next = now + 100; }
    socket.wait( 20 );
    for ( unsigned i = 0; i < 64; ++i ) {
      auto packet = socket.receive(); if ( !packet ) break;
      auto event = client.receive( packet->peer, packet->bytes, session_time_ms() );
      if ( event.reply ) socket.send( client.server(), *event.reply );
    }
  }
}
}
int main( int argc, char** argv )
{
  try {
    ...
    const bool pipe = argc == 2 && ( std::string_view( argv[1] ) == "close" || std::string_view( argv[1] ) == "leave" );
    if ( !ssh && !pipe ) throw Error( "invalid test driver arguments" );
    auto offer = [&]() {
      if ( pipe ) return from_pipe();
      SshStartupOptions options;
      options.ssh_path = argv[2]; options.config_path = argv[3];
      options.server_path = argv[4]; options.destination = "fixture";
    ...
    client.revalidate_path(); await_state( client, replacement, ClientState::active );
    ...
    std::cout << "PASS live session confirmation and port migration" << (ssh || std::string_view(argv[1])=="close" ? ", closed\n" : ", left active\n");
    return 0;
