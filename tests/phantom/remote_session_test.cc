...
UdpEndpoint endpoint( unsigned int port = 60000 )
{
  return UdpEndpoint::parse( "127.0.0.1", static_cast<std::uint16_t>( port ) );
}
RemotePolicy policy()
{
  RemotePolicy p;
...
{
  auto p = policy();
  RemoteSession server( key(), 0, p );
  SessionClient client( endpoint(), key(), 0 );
...
{
  RemoteSession server( key(), 0, policy() );
  SessionClient client( endpoint(), key(), 0 );
  auto from = endpoint( 40000 );
  rejects( [&] { client.send( {}, 0 ); } );
  rejects( [&] { server.send( {}, 0 ); } );
  handshake( server, client, from, 0 );
  auto early = client.send( { 1, 2 }, 1 ), later = client.send( { 3, 4 }, 2 );
  auto b = server.receive( from, later, 2 );
  auto a = server.receive( from, early, 2 );
  CHECK( b.data == std::optional<Bytes>( Bytes { 3, 4 } ) && a.data == std::optional<Bytes>( Bytes { 1, 2 } ) );
  CHECK( !server.receive( from, early, 2 ).data );
  Bytes payload( SESSION_MAX_DATA, 0x51 );
  auto out = server.send( payload, 3 );
  ...
  rejects( [&] { server.send( Bytes( SESSION_MAX_DATA + 1 ), 3 ); } );
  CHECK( server.state() == RemoteState::active );
  rejects( [&] { client.send( Bytes( SESSION_MAX_DATA + 1 ), 3 ); } );
...
void forged_path_and_lost_ready()
{
  RemoteSession server( key(), 0, policy() );
  SessionClient client( endpoint(), key(), 0 );
  auto from = endpoint( 40000 ), attacker = endpoint( 40001 );
  auto challenge = server.receive( from, client.retry( 0 ), 0 );
  ...
  auto confirm = client.receive( endpoint(), challenge.reply->datagram, 0 );
  CHECK( confirm.reply );
  CHECK( !server.receive( attacker, *confirm.reply, 0 ).reply );
  CHECK( !server.peer() );
  ...
  auto ready = server.receive( from, client.retry( 1 ), 1 );
  CHECK( ready.reply && ready.activated );
  CHECK( client.state() == ClientState::connecting ); // first READY deliberately lost
  ready = server.receive( from, client.retry( 2 ), 2 );
  CHECK( ready.reply && !ready.activated );
  client.receive( endpoint(), ready.reply->datagram, 2 );
  CHECK( client.state() == ClientState::active );
}
void expired_challenge_recovers()
{
  ...
  for ( std::uint64_t t = 101; t < 111 && client.state() != ClientState::active; ++t ) {
    ...
    if ( c.reply ) {
      auto ready = server.receive( from, *c.reply, t );
      CHECK( ready.reply );
...
void migration_and_sleep()
{
  RemoteSession server( key(), 0, policy() );
  SessionClient client( endpoint(), key(), 0 );
  auto old = endpoint( 40000 ), moved = endpoint( 40001 );
  handshake( server, client, old, 0 );
  auto delayed = client.send( { 9 }, 1 );
  client.revalidate_path();
  auto h = client.retry( 2 );
  auto challenge = server.receive( moved, h, 2 );
  CHECK( challenge.reply );
  CHECK( server.peer() == std::optional<UdpEndpoint>( old ) );
  CHECK( server.receive( old, delayed, 2 ).data == std::optional<Bytes>( Bytes { 9 } ) );
  auto proof = client.receive( endpoint(), challenge.reply->datagram, 2 );
  CHECK( proof.reply );
  auto ready = server.receive( moved, *proof.reply, 2 );
  CHECK( ready.migrated && !ready.activated );
  client.receive( endpoint(), ready.reply->datagram, 2 );
  CHECK( server.peer() == std::optional<UdpEndpoint>( moved ) );
  ...
  server.tick( six_months );
  CHECK( server.state() == RemoteState::active );
  client.revalidate_path();
  handshake( server, client, endpoint( 40002 ), six_months );
}
void client_exhaustion_is_closed()
{
  Policy crypto;
  crypto.rekey_packets = 1;
  crypto.hard_packets = 1;
  SessionClient client( endpoint(), key(), 0, crypto );
  client.retry( 0 );
  client.retry( 0 ); // epoch 0, then one unacknowledged epoch 1
  rejects( [&] { client.retry( 0 ); } );
  CHECK( client.state() == ClientState::closed );
  client.stop();
  client.tick( 1 );
  CHECK( client.state() == ClientState::closed );
}
void close_and_idle()
{
  ...
  auto from = endpoint( 40000 );
  handshake( server, client, from, 0 );
  auto delayed = client.send( { 1 }, 1 );
  auto closed = server.receive( from, client.close( 2 ), 2 );
  CHECK( closed.reply && closed.closing && server.state() == RemoteState::draining );
  CHECK( !server.receive( from, delayed, 2 ).data );
  for ( std::uint64_t t = 3; t < 52; ++t ) {
    auto ack = server.receive( from, client.retry( t ), t );
    CHECK( ack.reply );
  }
  server.tick( 52 );
  CHECK( server.state() == RemoteState::closed );
  client.receive( endpoint(), closed.reply->datagram, 52 );
  CHECK( client.state() == ClientState::closed );
  rejects( [&] { client.retry( 52 ); } );
  p.idle_ms = 50;
  RemoteSession idle( key(), 0, p );
  SessionClient c( endpoint(), key(), 0 );
  handshake( idle, c, from, 0 );
  auto stale = c.send( { 1 }, 1 );
  CHECK( idle.receive( from, c.send( { 2 }, 2 ), 2 ).data );
  CHECK( idle.receive( from, stale, 40 ).data ); // reordered data must not renew idle lease
  idle.tick( 52 );
  CHECK( idle.state() == RemoteState::closed );
}
}
int main()
{
  try {
    addresses();
    startup_cannot_be_renewed();
    data_and_reorder();
    forged_path_and_lost_ready();
    expired_challenge_recovers();
    migration_and_sleep();
    close_and_idle();
    client_exhaustion_is_closed();
    std::cout << "PASS numeric endpoints, startup expiry, return-path proof, loss recovery, migration, reordering, "
                 "logical sleep, closure and idle policy\n";
    return 0;
