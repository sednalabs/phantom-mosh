...
void await( SessionChannel& c, ChannelState wanted )
{
  for ( ;; ) {
    ...
    if ( c.state() == ChannelState::closed )
      throw Error( "test channel expired" );
    pollfd event { c.fd(), POLLIN, 0 };
    const int n = ::poll( &event, 1, c.wait_ms( session_time_ms() ) );
    if ( ( n < 0 && errno != EINTR ) || ( event.revents & POLLNVAL ) )
      throw Error( "test wait failed" );
  }
}
}
int main( int argc, char** argv )
{
  try {
    ...
    auto offer = read_offer();
    auto socket = SessionSocket::client_to( offer.server );
    auto channel
      = SessionChannel::client( std::move( socket ), offer.server, std::move( offer.secret ), session_time_ms() );
    ...
    if ( std::string_view( argv[1] ) == "close" ) {
      ...
      await( *channel, ChannelState::closed );
      if ( channel->end_reason() != ChannelEnd::peer_closed )
        throw Error( "test close was not acknowledged" );
    }
