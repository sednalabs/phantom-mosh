...
int main()
{
  try {
    ...
    Socket first, roaming, attacker, target;
    sockaddr_in admitted {};
    auto ingest = [&] {
      auto packet = target.receive();
      auto opened = server.open( packet.first, 0 );
      if ( opened && opened->newest )
        admitted = packet.second;
      return opened;
    };
    const auto first_packet = client.seal( { 1 }, 0 ), delayed = client.seal( { 2 }, 0 );
    first.send( target, first_packet );
    CHECK( ingest() );
    CHECK( admitted.sin_port == first.address.sin_port );
    auto corrupt = first_packet;
    corrupt.back() ^= 1;
    attacker.send( target, corrupt );
    CHECK( !ingest() );
    CHECK( admitted.sin_port == first.address.sin_port );
    roaming.send( target, client.seal( { 3 }, 0 ) );
    CHECK( ingest() );
    CHECK( admitted.sin_port == roaming.address.sin_port );
    first.send( target, delayed );
    auto old = ingest();
    CHECK( old && !old->newest );
    CHECK( admitted.sin_port == roaming.address.sin_port );
    attacker.send( target, first_packet );
    CHECK( !ingest() );
    CHECK( admitted.sin_port == roaming.address.sin_port );
    target.send( roaming, server.seal( { 4 }, 0 ) );
    auto reply = roaming.receive();
    auto opened = client.open( reply.first, 0 );
    CHECK( opened && opened->payload == Bytes { 4 } );
